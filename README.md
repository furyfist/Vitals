...
4. Every few seconds, an evaluator compares the current window to that baseline and emits a **verdict** — `WARMING`, `STEADY`, `CHANGED`, or `INCONCLUSIVE` — as a metric and a log line, back in SigNoz.
